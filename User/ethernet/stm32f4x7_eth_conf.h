...
#ifdef USE_Delay
  #include "bsp_init.h"              /* Header file where the Delay function prototype is exported */
  #define _eth_delay_    delay_1ms   /* User can provide more timing precise _eth_delay_ function 
                                   in this example Systick is configured with an interrupt every 10 ms*/
#else
  #define _eth_delay_    ETH_Delay /* Default _eth_delay_ function with less precise timing */
#endif
//ETH_Delay-是ETH自带的延时函数，精度更高
...
#define LY1210_RESET_DELAY    ((uint32_t)50)
...
#define PHY_SR                 ((uint16_t)0x11) /* PHY status register Offset */
#define PHY_SPEED_STATUS       ((uint16_t)0x0008) /* PHY Speed mask */
#define PHY_DUPLEX_STATUS      ((uint16_t)0x0002) /* PHY Duplex mask */
