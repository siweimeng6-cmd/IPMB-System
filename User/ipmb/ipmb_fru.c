...
uint8_t IPMB_FRU_HandleControl(uint8_t control_req, const uint8_t* data, uint8_t data_len)
{
    ...
    case 0x00:  /* 断电 */
        ...
        /* 同步状态灯(PA15): 立即熄灭, 不等 PB5 硬件反馈(Power_Ctrl_Task 里
         * 要 MCU 上电满10s才开始监测、还要再等150ms防抖才更新) */
    ...
    case 0x07:  /* 风扇默认档位 —— data[0]直接当0-100占空比,同时下发给两路风扇
                 * (跟UART的Fan1/Fan2命令走同一个Fan_SetManualDuty入口,区别是UART能给两路
                 * 分别设不同值,这里一次只能给两路设成同一个值) */
        if (data_len >= 1) {
            g_fru_state.fan_default_level = data[0];
            Fan_SetManualDuty(FAN_CH1, data[0]);
            Fan_SetManualDuty(FAN_CH2, data[0]);
        }
