...
static void decode_response(const uint8_t* req, const uint8_t* resp, uint8_t resp_len)
{
    ...
    case IPMB_CMD_GET_SENSOR_READING: {
        /* 数据 3 字节(2026-07-20起新格式): d[0]=×100定点值LSB d[1]=MSB
         * d[2]: bit7=事件上报使能 bit6=保留 bits5:0=阈值状态(本次仍固定0) */
        if (resp_len < 7 + 3) {
            printf("[IPMB-A Master] Get Sensor Reading: short resp\r\n");
            break;
        }
        const uint8_t* d = &resp[7];
        int16_t raw = (int16_t)((uint16_t)d[0] | ((uint16_t)d[1] << 8));
        printf("[IPMB-A Master]   Reading=%.2f EventEn=%u ThStatus=0x%02X\r\n",
               raw / 100.0f, (d[2] >> 7) & 0x01, d[2] & 0x3F);
        break;
    }
