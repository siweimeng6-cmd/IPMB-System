...
void Board_ADDR90_temp(void)
{
		...
		{
			temp_result = temp_calculate(board_lm75a_temp0); //采集转换成实际温度
			decimal_2_integer(temp_result,board_temp0); //整数化 	
			sprintf((char *)flash_buf,"Board_ADDR90_temp:%d.%d \r\n" ,board_temp0[0],board_temp0[1]);
		}
		else
		{
			sprintf((char *)flash_buf,"ADDR90 TEMP read failed.\r\n");
		}
...
{ 		
		...
		{	
			temp_result = temp_calculate(board_lm75a_temp1); //采集转换成实际温度
			decimal_2_integer(temp_result,board_temp1); //整数化	
			sprintf((char *)flash_buf,"Board_ADDR92_temp:%d.%d\r\n" ,board_temp1[0],board_temp1[1]);
		}
				else
		{
			sprintf((char *)flash_buf,"ADDR92 TEMP read failed.\r\n");
		}
		strncat((char *)stPrintf_Buf.buf, flash_buf, sizeof(stPrintf_Buf.buf) - strlen((char *)stPrintf_Buf.buf) - 1);	/* 有边界追加,防越界踩坏全局内存 */
		memset(flash_buf, 0, 128);
}
...
{ 		
		float temp_result;	
		char flash_buf[128] = {0};
		if(board_lm75a_temp_ReadBytes((uint8_t *)board_lm75a_temp2,0,2,BOARD_LM75A92_ADDRESS)==1)
		{	
			temp_result = temp_calculate(board_lm75a_temp2); //采集转换成实际温度
			decimal_2_integer(temp_result,board_temp2); //整数化	
			//sprintf((char *)flash_buf,"Board_ADDR94_temp:%d.%d ℃\r\n" ,board_temp2[0],board_temp2[1]);
		}
				else
		{
			sprintf((char *)flash_buf,"ADDR94 TEMP read failed.\r\n");
		}
		strncat((char *)stPrintf_Buf.buf, flash_buf, sizeof(stPrintf_Buf.buf) - strlen((char *)stPrintf_Buf.buf) - 1);	/* 有边界追加,防越界踩坏全局内存 */
		memset(flash_buf, 0, 128);
}
