 ...
 * 2→V12(ADC PA6), 3→V0.75(ADC PA7+PB0平均), 4→Iout(BPD20550 PMBus) */
                                      ...
                                      uint8_t own_addr)
{
    ...
    if (req_len < 13) {
        ...
        if (record_id >= IPMB_OEM_SENSOR_COUNT) {
            ...
            case 3: record[1] = IPMB_OEM_SENSOR_TYPE_VOLT; break;  /* V0.75   */
                                          ...
                                          uint8_t own_addr)
{
    ...
    if (req_len < 7 || req[6] == 0) {
        ...
        if (IPMB_Sensor_Find(sensor_num, &e) != 0) {
             ...
             * reading 为 x100 定点数, 分辨率 0.01, 范围 ±327.67, 覆盖当前所有量程 */
            switch (sensor_num) {
            case 0x04:  /* 温度 0 - LM75A @ 0x90, 0.01C */
                Board_ADDR90_temp();
                /* board_temp0[0]=整数°C, board_temp0[1]=百分位(已是x100小数部分) */
                reading = (board_temp0[0] >= 0)
                            ? (int16_t)(board_temp0[0] * 100 + board_temp0[1])
                            : (int16_t)(board_temp0[0] * 100 - board_temp0[1]);
                break;
            case 0x20:  /* 温度 1 - LM75A @ 0x92 */
                Board_ADDR92_temp();
                reading = (board_temp1[0] >= 0)
                            ? (int16_t)(board_temp1[0] * 100 + board_temp1[1])
                            : (int16_t)(board_temp1[0] * 100 - board_temp1[1]);
                break;
            case 0x03:  /* 12V - PA6, 0.01V */
                reading = (int16_t)(stADC_Data.adc_voltage_val[0] * 100.0f + 0.5f);
                break;
            case 0x17:  /* 5V - PC0/VRD_MOS */
                reading = (int16_t)(stADC_Data.adc_voltage_val[4] * 100.0f + 0.5f);
                break;
            case 0x19:  /* 0.75V - DDRPHY 平均 (PA7+PB0) */
                reading = (int16_t)(((stADC_Data.adc_voltage_val[1] +
                                     stADC_Data.adc_voltage_val[2]) / 2.0f) * 100.0f + 0.5f);
                break;
            case 0x41:  /* 风扇档位 - PWM 占空比, 本身是整数百分比, 不做定点缩放 */
                reading = (int16_t)pwm_get_duty();
                break;
            case 0x07:  /* 12V/40A 电流 - BPD20550 PMBus(缓存值,周期性更新), 0.01A */
                reading = (int16_t)(g_bpd20550_current_A * 100.0f + 0.5f);
                break;
            default:
                break;
            }
            ...
            cc = IPMB_CC_OK;
            data[0] = (uint8_t)(reading & 0xFF);          /* 08h: LSB */
            data[1] = (uint8_t)((reading >> 8) & 0xFF);   /* 09h: MSB, 不再固定0x80 */
            data[2] = 0x80;   /* 0Ah: bit7=事件允许上报(原09h=0x80的语义搬到这一位),
                                      bit6=保留固定0, bit5:0=阈值状态占位(维持0,未接入) */
        }
    }
 ...
 *         这条路径行为完全不变。用真实地址0x20轮询的既有调用方(task_usart.c
 *         的 IPMB_PEM_Poll_Task、手动 ipmb 命令)因为回显的还是0x20,行为也不变。
 */
 ...
 *         固定 5 个传感器: 90Temp, 92Temp, V12, V0.75, Iout
 */
    ...
    for (i = 0; i < item_count && data_len < (uint8_t)(sizeof(resp_data) - 4); i++) {
        ...
        case IPMB_OEM_ITEM_SENSOR_INFO:  /* 0x11: BIT 传感器信息 */
            ...
            for (sensor_idx = 0; sensor_idx < IPMB_OEM_SENSOR_COUNT; sensor_idx++) {
                ...
                case 3: entry[1] = IPMB_OEM_SENSOR_TYPE_VOLT; break;  /* V0.75   */
        ...
        case IPMB_OEM_ITEM_SENSOR_VALUE:  /* 0x12: 所有 BIT 值 */
            item_len = IPMB_OEM_SENSOR_COUNT * 4;  /* 5 × 4 = 20 字节 */
            {
                ...
                for (sensor_idx = 0; sensor_idx < IPMB_OEM_SENSOR_COUNT; sensor_idx++) {
                    ...
                    case 3:  /* V0.75 — ADC PA7+PB0 两路平均 (index 1, 2) */
                        {
                            float v0 = stADC_Data.adc_voltage_val[1];
                            float v1 = stADC_Data.adc_voltage_val[2];
