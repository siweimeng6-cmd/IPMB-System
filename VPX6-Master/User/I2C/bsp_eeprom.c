...
*********************************************************************************************************
*/
uint8_t  Blade_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
	uint16_t i;
	/* 第1步：发起I2C总线启动信号 */
	i2c_Start();
	...
	{
		i2c_SendByte(_usAddress >> 8);/*地址大于一个字节时先传输高位字节*/
		if (i2c_WaitAck() != 0)
		{
			goto cmd_fail;	/* EEPROM器件无应答 */
		}
		i2c_SendByte(_usAddress);/*地址大于一个字节时后传输低位字节*/
		if (i2c_WaitAck() != 0)
		{
			goto cmd_fail;	/* EEPROM器件无应答 */
		}
...
/*
*********************************************************************************************************
*	函 数 名: Time_WriteBytes
*	功能说明: I2C每发送一个字节会得到一个一个应答位 0 
*	形    参:  _usAddress : 起始地址
*			       _usSize    : 数据长度,单位为字节
*			       _pWriteBuf : 存放读到的数据的缓冲区指针
*	返 回 值: 0 表示失败,1表示成功
*********************************************************************************************************
*/
uint8_t Time_WriteBytes(uint8_t *_pWriteBuf, uint16_t _usAddress, uint16_t _usSize)
{
	uint16_t i,m;
	uint16_t usAddr;
	/*
